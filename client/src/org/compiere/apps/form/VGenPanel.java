...
public class VGenPanel extends CPanel implements ActionListener, ChangeListener, TableModelListener, ASyncProcess
{
	...
	public void generateComplete(ProcessInfo pi)
	{
		...
		// @emmie - Controla si se procesaron facturas fiscales para
		//          evitar que se vuelvan a imprimir
		if (pi.getAD_Process_ID() == 134) // C_InvoiceCreate process
		{
		    for (int c_Invoice_ID : ids)
		    {
		        final MInvoice invoice = new MInvoice(Env.getCtx(), c_Invoice_ID, null);
		        if (invoice.get_ValueAsBoolean("IsFiscalPrinted") ||
		            invoice.getDocumentNo().startsWith("Anu-")) // controla las que se anularon
		            return;
		    }
		}
		// @emie fin
