...
public class MProduction extends X_M_Production implements DocAction, DocOptions {
	/**
	 * 
	 */
    ...
    public MProduction(MProjectLine line)
    {
        ...
        setAD_OrgTrx_ID(project.get_ValueAsInt("AD_OrgTrx_ID"));
        setC_Activity_ID(project.get_ValueAsInt("C_Activity_ID"));
    ...
    public String completeIt()
    {
        ...
        else
        {
            ...
            for (MProductionPlan plan : plans)
            {
                ...
                if (!isHaveEndProduct(lines))
                {
                    m_processMsg = String.format("Production plan (line %1$d id %2$d) does not contain End Product",
                            plan.getLine(), plan.get_ID());
                    return DocAction.STATUS_Invalid;
                }
    ...
    protected int createLines(boolean mustBeStocked, MProduct finishedProduct, BigDecimal requiredQty)
    {
		...
		String sql = "SELECT M_ProductBom_ID, BOMQty"
		           +  " FROM M_Product_BOM"
				   + " WHERE M_Product_ID=" + finishedProduct.getM_Product_ID()
				   + " ORDER BY Line";
            ...
            while (rs.next())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        MProductCategory pc = MProductCategory.get(getCtx(), usedProduct.getM_Product_Category_ID());
    ...
    public String prepareIt()
    {
        ...
        {
            Query planQuery = new Query(getCtx(), I_M_ProductionPlan.Table_Name, "M_ProductionPlan.M_Production_ID=?",
                    get_TrxName());
            List<MProductionPlan> plans = planQuery.setParameters(getM_Production_ID()).list();
            for (MProductionPlan plan : plans)
            {
    ...
    protected String validateEndProduct(int M_Product_ID)
    {
        ...
        try
        {
            if (!costsOK(M_Product_ID))
            {
                ...
                if (MSysConfig.getBooleanValue("LAR_ValidateCostsDifferenceOnCreate", false, getAD_Client_ID()))
                {
                    return msg;
                }
                else
    ...
    protected boolean costsOK(int M_Product_ID) throws AdempiereUserError
    {
        ...
        if (MAcctSchema.COSTINGMETHOD_StandardCosting.equals(costingMethod))
        {
            String sql = "SELECT ABS(((cc.currentcostprice-(SELECT SUM(c.currentcostprice*bom.bomqty)"
                    + " FROM m_cost c" + " INNER JOIN m_product_bom bom ON (c.m_product_id=bom.m_productbom_id)"
            ...
            if (costPercentageDiff == null)
            {
                ...
                if (MSysConfig.getBooleanValue("LAR_ValidateCostsOnCreate", false, getAD_Client_ID()))
                {
    ...
    protected MProduction reverse(boolean accrual)
    {
        ...
        for (int i = 0; i < sLines.length; i++)
        {
            ...
            if (sLines[i].getM_AttributeSetInstance_ID() == 0)
            {
                ...
                {
                    MProductionLineMA ma = new MProductionLineMA(tLines[i], mas[j].getM_AttributeSetInstance_ID(),
