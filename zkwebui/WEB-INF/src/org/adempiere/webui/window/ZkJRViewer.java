...
public class ZkJRViewer extends Window implements EventListener, ITabOnCloseHandler {
	...
	private void init() {
	    ...
	    previewType.appendItem("TXT", "TXT"); // @Ergio - Se agrega la opción TXT
		...
		try {
            ...
            String titulo = new String(jasperPrint.getName());
            AMedia media = new AMedia(titulo, "pdf", "application/pdf", file, true);
	...
	private void renderReport() throws Exception {
        ...
        // @Ergio - Se agrega la opción de exportación a TXT
		}else if ("TXT".equals(previewType.getSelectedItem().getValue())) {
            String path = System.getProperty("java.io.tmpdir");
            String prefix = makePrefix(jasperPrint.getName());
            if (log.isLoggable(Level.FINE))
            {
            ...
            File file = File.createTempFile(prefix, ".txt", new File(path));
            FileOutputStream fos = new FileOutputStream(file);
            JRTextExporter jrtxt = new JRTextExporter();
            jrtxt.setParameter(JRExporterParameter.JASPER_PRINT, jasperPrint);
            jrtxt.setParameter(JRExporterParameter.OUTPUT_STREAM,  fos);
